...
def main(root_path, data_sampled_path, data_path, fs_method, index_col, fold_col, target_col):
    ...
    for ml_method in ml_methods:
        if ml_method == "KNN":
            model = KNeighborsRegressor(n_neighbors=math.floor(math.sqrt(x_sampled.shape[0])))
        if ml_method == "OLS":
        ...
        if ml_method == "SVM":
            model = SVR()
        if ml_method == "MLP":
            model = MLPRegressor((math.floor((x_sampled.shape[1] + 1)/2),), random_state=1, 
                                  max_iter=50000, 
    ...
    results.to_csv(os.path.join(data_sampled_path, "out_of_sample_error.csv"), index=False)
