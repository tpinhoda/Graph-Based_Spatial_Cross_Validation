...
def read_data(root_path, dataset_name, index_col, target_col, fold_col):
    data = pd.read_csv(join(root_path, dataset_name, "data.csv"))
    reorganized_cols = [
        col for col in data.columns if col not in [target_col, fold_col]
    ]
...
def _save_selected_features(output_path ,features, fold):
    ...
    with open(
        join(output_path, f"{fold}.json"), "w", encoding="utf-8"
    ) as file:
